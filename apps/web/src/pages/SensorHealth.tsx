...
export default function SensorHealth() {
  ...
  const log = [...events].filter((e) => e.kind === 'sensor' || (e.kind === 'director' && e.text.includes('fault'))).reverse().slice(0, 12);

  return (
    <div className="scroll-thin h-full space-y-3 overflow-y-auto p-3">
      <div className="grid grid-cols-6 gap-3">
        <div className="panel col-span-2 flex items-center gap-3 p-3">
          ...
          <div>
            ...
            <div className="text-[11px] text-slate-400">Bad inputs are auto-excluded, then re-admitted after probation.</div>
          </div>
        </div>
        {counts.map((c) => (
        ...
        <div className="panel min-w-0 p-3">
          <div className="mb-2 flex flex-wrap items-center gap-1">
            {(['all', 'dwr', 'satellite', 'lightning', 'aws', 'nwp'] as const).map((k) => (
              <button key={k} onClick={() => setKind(k)} className={`rounded-md px-2 py-0.5 text-xs ${kind === k ? 'bg-volt/20 text-volt' : 'text-slate-400 hover:bg-white/5'}`}>
              ...
              {list.map((s) => {
                ...
                return (
                  <tr key={s.id} className="border-t border-white/5">
                    ...
                    <td className="text-right font-mono tnum">{s.latencySec < 120 ? `${s.latencySec.toFixed(0)} s` : `${(s.latencySec / 60).toFixed(1)} min`}</td>
                    ...
                    <td className="text-right">
                      <div className="ml-auto flex w-24 items-center gap-1.5">
                        ...
                        <span className="w-8 font-mono text-[11px] tnum">{Math.round(s.trust * 100)}</span>
                      </div>
                    </td>
                    <td className="pl-3">
        ...
        <div className="space-y-3">
          <div className="panel p-3">
            ...
            <ul className="space-y-1.5 text-[12px] text-slate-300">
              <li><b className="text-white">Spike</b> — reading beyond 4× robust MAD of the last 15 values</li>
              <li><b className="text-white">Frozen</b> — 8 identical readings in a row</li>
              <li><b className="text-white">Drift</b> — offset vs neighbour consensus &gt; 2.5 units</li>
              <li><b className="text-white">Dropout</b> — no data for &gt; 3× the expected latency</li>
              <li className="text-slate-400">Trust &lt; 45% → excluded from fusion; after 10 min clean → back to OK.</li>
            </ul>
          </div>
          <div className="panel p-3">
            <div className="panel-title mb-2">Self-healing log</div>
            <div className="space-y-1">
              {log.map((e) => (
                <div key={e.id} className="text-[11px] leading-snug text-slate-300">
                  {e.text}
                </div>
              ))}
              {!log.length && <div className="text-xs text-slate-500">All feeds healthy. Use Director Mode (Shift+D) to inject a fault.</div>}
            </div>
          </div>
          <div className="panel p-3">
            ...
            <div className="space-y-2">
              {FEED_ADAPTERS.map((f) => (
                <div key={f.id} className="rounded-lg bg-white/[0.03] p-2 text-[11px]">
                  ...
                  <div className="text-slate-400">{f.product} · {f.format} · {f.cadence}</div>
