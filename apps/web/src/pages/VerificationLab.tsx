...
export default function VerificationLab() {
  ...
  const relOpt = useMemo(() => {
    ...
    return {
      ...
      series: [
        { type: 'line', data: [[0, 0], [1, 1]], symbol: 'none', lineStyle: { color: '#475569', type: 'dashed' }, silent: true },
        { type: 'line', name: 'VAJRA 60 min', data: rel.map((r) => [+r.forecast.toFixed(3), +r.observed.toFixed(3), r.count]), symbolSize: (d: number[]) => 4 + Math.min(14, Math.log10(d[2] + 1) * 4), lineStyle: { color: '#22d3ee', width: 2.5 }, itemStyle: { color: '#22d3ee' } },
        { type: 'bar', data: rel.map((r) => [+r.forecast.toFixed(3), Math.min(1, Math.log10(r.count + 1) / 6)]), barWidth: 6, itemStyle: { color: 'rgba(167,139,250,0.3)' }, silent: true },
      ],
    };
  }, [v?.reliability]);
    ...
    <div className="scroll-thin h-full space-y-3 overflow-y-auto p-3">
      <div className="panel flex items-center justify-between p-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">Every 10 min each method issues 30/60/120/180-min forecasts; each is scored when its valid time arrives (≥35 dBZ event, 8 km grid, 1-pixel tolerance).</div>
          </div>
        </div>
        ...
        <div className="panel p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              {METRICS.map((m) => (
                <button key={m} onClick={() => setMetric(m)} className={`rounded-md px-2 py-0.5 font-mono text-xs ${metric === m ? 'bg-volt/20 text-volt' : 'text-slate-400 hover:bg-white/5'}`}>
                  {M_LABEL[m]}
                </button>
              ))}
            </div>
          ...
          <div className="mt-1 text-[11px] text-slate-500">{LOWER_BETTER.includes(metric) ? 'Lower is better.' : 'Higher is better.'} Scores are exponentially weighted over recent verifications, so they drift with the weather.</div>
        </div>
        <div className="panel p-3">
      ...
      <div className="panel p-3">
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="flex gap-1">
            {[30, 60, 120, 180].map((l) => (
              <button key={l} onClick={() => setLead(l)} className={`rounded-md px-2 py-0.5 font-mono text-xs ${lead === l ? 'bg-volt/20 text-volt' : 'text-slate-400 hover:bg-white/5'}`}>
                T+{l}
              </button>
            ))}
          </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-wider text-slate-500">
              ...
              {METRICS.map((m) => (
                <th key={m} className="text-right">{M_LABEL[m]}</th>
              ))}
        ...
        <div className="mt-3 flex items-start gap-2 rounded-lg bg-white/[0.03] p-2.5 text-[12px] text-slate-300">
          ...
          <span>
            CSI skill of VAJRA over optical flow at T+{lead}: <b className="font-mono text-white">{(skill * 100).toFixed(0)}%</b>. VAJRA adds a growth/decay term and a convective-initiation term to plain advection, which is why its advantage grows with lead time. New storms that have not formed yet stay the main source of misses — no method is near-perfect beyond 2 h.
          </span>
        </div>
      </div>
    </div>
