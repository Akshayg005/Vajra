    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      {pickMode && (
        <div className="pointer-events-none absolute left-1/2 top-4 z-10 -translate-x-1/2 rounded-full border border-plasma/50 bg-plasma/20 px-4 py-1.5 text-sm font-semibold text-white shadow-violet">
          Director: click on the map to {pickMode === 'spawn' ? 'spawn a storm cell' : 'place a report'}
        </div>
      )}
      <div className="pointer-events-none absolute inset-0 flex gap-3 p-3">
        <div className="scroll-thin flex flex-col gap-3 overflow-y-auto pb-20">
          <LayerPanel />
          <Legend />
          <EventFeed />
        </div>
        <div className="flex-1" />
        <div className="flex max-h-full flex-col gap-3 pb-20">
          {selected ? <CellInspector /> : <CellList />}
        </div>
      </div>
