      ...
      [...reports]
        ...
        .filter(
          (r) =>
            ev.has(r.event) &&
            st.has(r.status) &&
            simTime - r.t <= hours * 3600e3 &&
            (!q || r.place.toLowerCase().includes(q.toLowerCase()) || r.text.toLowerCase().includes(q.toLowerCase())),
        ),
        ...
        <div className="space-y-2 border-b border-white/5 p-3">
          ...
          <div className="flex flex-wrap gap-1">
            {EVENTS.map((e) => (
              <button
                key={e}
                onClick={() => toggle(ev, e, setEv)}
                className={`rounded-md border px-2 py-0.5 text-xs capitalize ${ev.has(e) ? 'border-volt/50 bg-volt/10 text-white' : 'border-white/10 text-slate-500'}`}
              >
                {e}
              </button>
            ))}
          ...
          <div className="flex flex-wrap gap-1">
            {STATUS.map((e) => (
              <button
                key={e}
                onClick={() => toggle(st, e, setSt)}
                className={`rounded-md border px-2 py-0.5 text-xs capitalize ${st.has(e) ? 'border-plasma/50 bg-plasma/10 text-white' : 'border-white/10 text-slate-500'}`}
              >
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Filter by place or text"
              className="flex-1 rounded-md border border-white/10 bg-ink-800 px-2 py-1 text-xs outline-none focus:border-volt/60"
            />
        ...
        <div className="scroll-thin min-h-0 flex-1 overflow-y-auto">
          {list.map((r) => {
            ...
            return (
              <button
                key={r.id}
                onClick={() => activeMap?.flyTo({ center: [r.lng, r.lat], zoom: 9 })}
                className="w-full border-b border-white/[0.04] px-3 py-2 text-left hover:bg-white/5"
              >
